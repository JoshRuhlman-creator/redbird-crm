...
function parseAccounts(rows) {
  ...
  // (Cannazone, Green Lady, Kush 21, etc.) where Cultivera reports the parent
  // brand as Client but the License # identifies the specific store.
  const licenseLookup = {};
  ...
  rows.filter(r => r['Trade Name']).forEach(r => {
    ...
    const city    = String(r['City'] || r['Mailing City'] || '').trim();
    if (license) {
    ...
    // CRM Accounts tab is deduped by lowercase Trade Name during rebuild — so
    // multi-location brands collapse to ONE entry here. Per-store routing
    // happens later via licenseLookup + the order's License # column.
    accountData[tradeName] = {
...
function enrichAccountsAndBuildKPIs(accountData, opps) {
  ...
  opps.forEach(o => {
    ...
    // Cultivera reports the parent Trade Name as Client (e.g. "Cannazone") for
    // every store — but each store has its own License #. CRM Accounts has all
    // 7 Cannazone rows but they collapse to one entry under "Cannazone" since
    // dedupe is by name. Meanwhile the user has per-store stubs like
    // "Cannazone Burlington", "Cannazone Sodo" from manual contact entries.
    // This routing builds the per-store name from license → city and routes
    // there if the account exists.
    if (o.license && window.LICENSE_LOOKUP) {
      var rec = window.LICENSE_LOOKUP[String(o.license).trim()];
      if (rec && rec.city) {
        var candidate = (rec.tradeName + ' ' + rec.city).trim();
        // Direct match
        if (accountData[candidate]) {
          name = candidate;
          o.a = name;
          routedByLicense = true;
          licenseRerouted++;
        } else if (acctNamesLower[candidate.toLowerCase()]) {
          // Case-insensitive
          name = acctNamesLower[candidate.toLowerCase()];
          o.a = name;
          routedByLicense = true;
          licenseRerouted++;
        } else {
          // Normalized — catches "Cannazone Hwy 99" when city is "Mount Vernon"
          // through user-defined naming. Won't help that specific case but does
          // help "Cannazone Bellingham" → CRM has "Cannazone Bellingham" stub.
          var normCandidate = _normalizeAcctName(candidate);
          if (normCandidate && acctNamesNormalized[normCandidate]) {
            name = acctNamesNormalized[normCandidate];
            o.a = name;
            routedByLicense = true;
            licenseRerouted++;
          }
  ...
  // ── Cleanup: Remove stub accounts (created by parseOrders fallback) with no orders in 12 months ──
  // NEVER remove stubs that have recent revenue — they're real accounts with name mismatches.
  // Threshold tightened from 3 years → 12 months to actually trim dead weight.
  const cutoffDate = new Date();
  cutoffDate.setMonth(cutoffDate.getMonth() - 12);
  const cutoff12mo = cutoffDate.toISOString().slice(0, 10);
  let removed = 0;
  const removedNames = [];
  ...
  Object.keys(accountData).forEach(name => {
    ...
    // Never delete if it has any orders in the last 12 months
    const lastOrder = acct.stats.lastOrder || '';
    if (lastOrder >= cutoff12mo) return;
  ...
  if (removed > 0) {
    console.log(`[Sheets Loader] Cleaned up ${removed} stale stub accounts (no orders in 12 months, not in CRM Accounts)`);
...
function mergeAccountsByAddress(accountData, kpis, opps) {
  ...
  // Bucket every account by address key
  var byAddr = {};
  ...
  console.log('[Sheets Loader] Merged', merges.length, 'address-duplicate groups');
  merges.forEach(function(m) {
