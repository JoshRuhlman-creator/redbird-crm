...
function parseAccounts(rows) {
  ...
  // same Trade Name), the 2nd+ occurrences must NOT overwrite the first —
  // each store needs its own accountData entry.
  const tradeNameSeen = {};
  ...
  rows.filter(r => r['Trade Name']).forEach(r => {
    ...
    //   • First time we see a Trade Name → key = Trade Name (unchanged)
    //   • 2nd+ time (multi-location brand) → key = "Trade Name [license]" so
    //     each store survives as its own account instead of overwriting.
    // The matcher later routes by license, and the display layer can clean up
    // the "[license]" suffix; the underlying key just has to be unique.
    tradeNameSeen[tradeName] = (tradeNameSeen[tradeName] || 0) + 1;
    let acctKey = tradeName;
    if (tradeNameSeen[tradeName] > 1) {
...
function enrichAccountsAndBuildKPIs(accountData, opps) {
  ...
  // parseAccounts keyed extra stores as "Trade Name [license]" (e.g.
  // "Uncle Ike's [421514]") so each store survived dedupe. Now that orders are
  // matched, rename each such account to the actual Cultivera Client string
  // (e.g. "UNCLE IKES-White Center") — that's the name the user recognizes and
  // the name on every order. We pick the most common Client name seen for that
  // license's orders.
  (function renameLicenseAccounts() {
    ...
    opps.forEach(function(o) {
      ...
      // o.a was set to the routed key; recover the original Cultivera Client
      // from o._rawClient if present, else skip (handled below)
      var cn = o._rawClient || '';
      if (!cn) return;
    ...
    Object.keys(clientNamesByKey).forEach(function(oldKey) {
      // Only rename the "[license]"-suffixed keys — leave first-occurrence
      // accounts (plain Trade Name) alone.
      if (!/\s\[.+\]$/.test(oldKey)) return;
      var counts = clientNamesByKey[oldKey];
      // Most common Client name wins
      var newKey = Object.keys(counts).sort(function(a,b){ return counts[b]-counts[a]; })[0];
      if (!newKey || newKey === oldKey) return;
      // Avoid clobbering an existing distinct account
      if (accountData[newKey] && newKey !== oldKey) return;
      ...
      if (lic) window.LICENSE_TO_ACCT[String(lic).trim()] = newKey;
