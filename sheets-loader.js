...
function parseInventory(rawRows) {
  ...
  const results = dataRows
    ...
    .map(r => {
      ...
      // Col mapping (from actual Product Allocation sheet):
      //  0: Week of date     1: Room       2: Totes      3: Plant Count  4: Total Days
      //  5: Strain           6: Flower Del 7: Joint Del
      //  8: Per Tote         9: (%)       10: Proj 3.5g  11: (?)   12: (?)
      // 13: Target 28g      14: (?)       15: Proj Micros 14g
      // 16-17: (empty)      18: (trim value)
      // 19: (% field)       20: 1g joints  21: 2pk joints  22: 10pk joints
      // 23: Infused checkbox (TRUE/FALSE) — infused prerolls available
      // 24: Batch Count     25: Total Proj 1g   26: Total Proj 0.5g
      // 27: (empty)         28: Fresh Frozen checkbox (TRUE/FALSE) — going to live resin
      // 29: Cured checkbox
      ...
      var infused = String(r[23] || '').toUpperCase() === 'TRUE';       // Col X (23) = Infused prerolls
      var isFreshFrozen = String(r[28] || '').toUpperCase() === 'TRUE'; // Col AC (28) = Fresh Frozen = Live Resin
      ...
      return {
        ...
        fd:      normDate(r[6]),
        jd:      normDate(r[7]),
        p35:     Math.round(fnum(r[10])),
        a35:     0,
        pm:      Math.round(fnum(r[15])),
        j1:      proj1g,     // 1g prerolls
        j2:      proj2pk,    // 2pk joints
        j10:     proj10pk,   // 10pk joints
        inf:     isFreshFrozen ? 1 : 0,  // Fresh Frozen = going to live resin (no flower/joints)
        infused: infused ? 1 : 0,        // Infused prerolls available for this strain
        perTote:      num(r[3]),
        ...
        qa:           0,
        cured:        String(r[29] || '').toUpperCase() === 'TRUE' ? 1 : 0,
      };
    });
