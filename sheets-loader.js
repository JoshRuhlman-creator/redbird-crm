...
const SHEETS_CONFIG = {
  ...
  archiveSheetId: '1GCvS1EK8u6YmyKaZEIWA7omUSlZsapktEjjIcWDEL1g',
  tabs: {
...
// Fetch from the archive sheet (different spreadsheet)
function archiveSheetsUrl(tab) {
  const base = 'https://sheets.googleapis.com/v4/spreadsheets';
  const enc  = encodeURIComponent(tab);
  return `${base}/${SHEETS_CONFIG.archiveSheetId}/values/${enc}?key=${SHEETS_CONFIG.apiKey}&valueRenderOption=FORMATTED_VALUE`;
}

async function fetchArchiveTab(tab) {
  const res = await fetch(archiveSheetsUrl(tab));
  if (!res.ok) throw new Error(`Failed to fetch archive tab "${tab}": ${res.status}`);
  const json = await res.json();
  const [headers, ...rows] = json.values || [];
  return rows.map(row => {
    const obj = {};
...
// ─────────────────────────────────────────────
// ARCHIVE DATA PARSER → window.ARCHIVE_DATA
// Keyed by "weekDate|strain" → actual packaged unit counts
// ─────────────────────────────────────────────
...
function parseArchive(rows) {
  ...
  rows.forEach(function(r) {
    var weekDate = normDate(r['Week of Date']);
    var strain = (r['Strain'] || '').trim();
    if (!weekDate || !strain) return;

    var key = weekDate + '|' + strain.toLowerCase();
    ...
    //  BW: 2-pk Units → j2
    //  BX: 10-pk Units → j10
    //  BY: 1-g Infused Units → inf
    //  BZ: 5-pk Infused Units → inf5
    ...
    var entry = {
      ...
      totes: num(r['Totes']),
      room: num(r['Grow Room']),
      p35: num(r['3.5-g']),
      target7g: num(r['7-g']),
      target28g: num(r['28-g']),
      pm: num(r['14g Micros'] || r['14-g Micros']),
      j1: num(r['1-g Units']),
      j2: num(r['2-pk Units']),
      j10: num(r['10-pk Units']),
      inf: num(r['1-g Infused Units']),
      inf5: num(r['5-pk Infused Units']),
      hasActuals: true,
    };

    // If this week+strain already exists, aggregate (multiple totes same week/strain)
    if (archive[key]) {
      var ex = archive[key];
      ex.p35 += entry.p35;
      ex.target7g += entry.target7g;
      ex.target28g += entry.target28g;
      ex.pm += entry.pm;
      ex.j1 += entry.j1;
      ex.j2 += entry.j2;
      ex.j10 += entry.j10;
      ex.inf += entry.inf;
      ex.inf5 += entry.inf5;
      ex.totes += entry.totes;
    } else {
      archive[key] = entry;
    }
  });

  console.log('[Sheets Loader] Archive parsed:', Object.keys(archive).length, 'unique week+strain entries');
  return archive;
}
...
function parseInventory(rawRows) {
  ...
  const results = dataRows
    ...
    .map(r => {
      ...
      // 19: Proj Total Joints  20-21: (zeroes)  22: has-joints flag
      // 23-24: (zeroes)     25-26: (empty)   27: (value field)
      // 28: Infused available (TRUE/FALSE)
      // 29-30: (zeroes)     31: (value field)
      ...
      var isInfused = String(r[23] || '').toUpperCase() === 'TRUE'; // Col X (23) = Infused prerolls checkbox
      var isFreshFrozen = String(r[27] || '').toUpperCase() === 'TRUE'; // Col AB (27) = Fresh Frozen checkbox → live resin
      ...
      return {
        ...
        inf:     isInfused ? 1 : 0, // infused preroll flag
        liveResin: isFreshFrozen, // boolean: this strain goes to live resin vape (fresh frozen)
        perTote:      num(r[3]),
...
function injectIntoApp(accountData, kpis, opps, inventory, currentInv, pipeline, sheetTasks) {
  ...
  if (sheetTasks && sheetTasks.length && typeof window.tasks !== 'undefined') {
    ...
    // Re-apply localStorage task edits (date changes, name changes) and done state
    var doneMap = {};
    try { doneMap = JSON.parse(localStorage.getItem('rb_tasks_done')||'{}'); } catch(e) {}
    var taskEdits = {};
    try { taskEdits = JSON.parse(localStorage.getItem('rb_task_edits')||'{}'); } catch(e) {}

    var doneApplied = 0;
    window.tasks.forEach(function(t) {
      var originalKey = (t.accountName + '|' + t.taskName + '|' + t.dueDate).toLowerCase();

      // Apply persisted edits
      var edit = taskEdits[originalKey];
      if (edit) {
        if (edit.dueDate) t.dueDate = edit.dueDate;
        if (edit.taskName) t.taskName = edit.taskName;
        if (edit.typeClass) { t.typeClass = edit.typeClass; }
        if (edit.rule) t.rule = edit.rule;
        if (edit.notes !== undefined) t.notes = edit.notes;
      }

      // Check done state with both original and updated keys
      var updatedKey = (t.accountName + '|' + t.taskName + '|' + t.dueDate).toLowerCase();
      if ((doneMap[originalKey] || doneMap[updatedKey]) && !t.done) {
        t.done = true;
        doneApplied++;
      }
      ...
      t.isOverdue = t.dueDate < todayStr && !t.done;
    });
  ...
  showBanner(
    ...
    + (window.ARCHIVE_DATA && Object.keys(window.ARCHIVE_DATA).length ? ' · ' + Object.keys(window.ARCHIVE_DATA).length + ' archive' : '')
  );
}
...
async function loadSheetsData() {
  ...
  try {
    ...
    // Load archive data (actual packaged numbers) from separate sheet
    let archiveData = {};
    try {
      const archiveRows = await fetchArchiveTab(SHEETS_CONFIG.tabs.archive);
      archiveData = parseArchive(archiveRows);
      console.log('[Sheets Loader] Archive loaded:', Object.keys(archiveData).length, 'entries');
    } catch(e) {
      console.warn('[Sheets Loader] Could not load archive:', e.message);
    }
    ...
    // Override projected inventory with actual archive data where available
    if (Object.keys(archiveData).length > 0) {
      var overrideCount = 0;
      inventory.forEach(function(row) {
        var key = (row.w + '|' + row.strain.trim().toLowerCase());
        var arch = archiveData[key];
        if (arch && arch.hasActuals) {
          // Only override if the archive has non-zero data (batch was actually processed)
          var hasPackagedData = arch.p35 > 0 || arch.pm > 0 || arch.j1 > 0 || arch.j2 > 0 || arch.j10 > 0 || arch.inf > 0;
          if (hasPackagedData) {
            row.p35 = arch.p35;
            row.pm = arch.pm;
            row.j1 = arch.j1;
            ...
            row.j10 = arch.j10;
            row.inf = arch.inf;
            if (arch.target7g > 0) row.target7g = arch.target7g;
            if (arch.target28g > 0) row.target28g = arch.target28g;
            row._fromArchive = true;  // flag for UI to show "actual" badge
            overrideCount++;
          }
        }
      });
      console.log('[Sheets Loader] Archive overrides applied:', overrideCount, 'batches updated with actual numbers');
    }

    // Expose archive data globally
    ...
    // ── Mirror current inventory to "batches in stock" tab on Product Allocation sheet ──
    if (currentInv && currentInv.length > 0) {
      try {
        var appsScriptUrl = (typeof ACTIVITY_LOG_URL !== 'undefined' && ACTIVITY_LOG_URL)
          ? ACTIVITY_LOG_URL
          : 'https://script.google.com/macros/s/AKfycbwE2S9KJ4AHLanpHa7Epsl8wytVh9qOmfxJg8TY0KY3FgGu2ExhHNWZbmwpbI4-5A5Sww/exec';
        fetch(appsScriptUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'text/plain' },
          body: JSON.stringify({
            action: 'mirrorBatchesInStock',
            data: currentInv
          })
        }).then(function(r) { return r.json(); }).then(function(result) {
          if (result.success) {
            console.log('[Sheets Loader] Batches in stock mirrored:', result.count, 'rows at', result.timestamp);
          } else {
            console.warn('[Sheets Loader] Batches in stock mirror failed:', result.error);
          }
        }).catch(function(err) {
          console.warn('[Sheets Loader] Batches in stock mirror error:', err);
        });
      } catch(e) {
        console.warn('[Sheets Loader] Could not mirror batches in stock:', e);
      }
    }
