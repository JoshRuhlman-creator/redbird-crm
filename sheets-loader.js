...
function mergeContacts(accountData, contactRows) {
  ...
  // Build multiple lookup maps for matching contacts to accounts
  // 1. License number → account name (most reliable)
  const licenseMap = {};
  // 2. Normalized address → account name
  const addressMap = {};
  // 3. Account name (case-insensitive) → account name
  const nameMap = {};

  Object.keys(accountData).forEach(name => {
    const a = accountData[name];
    // License lookup
    if (a.license) {
      licenseMap[a.license.toString().trim().toLowerCase()] = name;
    }
    // Address lookup — normalize: lowercase, strip whitespace, remove common abbreviations
    if (a.address && a.city) {
      var normAddr = (a.address + '|' + a.city).toLowerCase().replace(/\s+/g, ' ').trim();
      addressMap[normAddr] = name;
    }
    // Just address without city as fallback
    if (a.address) {
      var addrOnly = a.address.toLowerCase().replace(/\s+/g, ' ').trim();
      if (!addressMap[addrOnly]) addressMap[addrOnly] = name;
    }
    // Name lookup
  ...
  contactRows.forEach(row => {
    ...
    const title = (row['Title'] || row['Salutation'] || '').trim();
    const acctName = (row['Account Name'] || '').trim();
    const email = (row['Email'] || '').trim();
    const phone = (row['Phone'] || '').trim();
    const licenseNum = (row['License Number'] || row['License #'] || '').trim();
    ...
    // Try to find the account — priority order:
    // 1. License number match
    // 2. Mailing address + city match
    // 3. Mailing address only match
    // 4. Account name exact match
    // 5. Account name partial match
    let account = null;
    let matchMethod = '';

    // 1. License number
    if (licenseNum && licenseMap[licenseNum.toLowerCase()]) {
      account = accountData[licenseMap[licenseNum.toLowerCase()]];
      matchMethod = 'license';
    }

    // 2. Mailing address + city
    if (!account && mailingStreet && mailingCity) {
      var normAddr = (mailingStreet + '|' + mailingCity).toLowerCase().replace(/\s+/g, ' ').trim();
      if (addressMap[normAddr]) {
        account = accountData[addressMap[normAddr]];
        matchMethod = 'address+city';
      }
    }

    // 3. Mailing address only
    if (!account && mailingStreet) {
      var addrOnly = mailingStreet.toLowerCase().replace(/\s+/g, ' ').trim();
      if (addressMap[addrOnly]) {
        account = accountData[addressMap[addrOnly]];
        matchMethod = 'address';
      }
    }

    // 4. Account name exact (case-insensitive)
    if (!account && acctName) {
      if (nameMap[acctName.toLowerCase().trim()]) {
        account = accountData[nameMap[acctName.toLowerCase().trim()]];
        matchMethod = 'name-exact';
      }
    }

    // 5. Fuzzy name match — strip parentheses, common suffixes, score by word overlap
    if (!account && acctName) {
      // Normalize: remove parentheses, common words, lowercase
      function normName(n) {
        return n.toLowerCase()
          .replace(/[()[\]{}\-_.,]/g, ' ')  // strip parens, brackets, dashes, dots
          .replace(/\b(cannabis|marijuana|recreational|dispensary|llc|inc|co|corp|the|of|and|&|club|company|store|shop|house|weed|pot|mary|jane|green|market)\b/gi, ' ')
          .replace(/[^a-z0-9\s]/g, ' ')
          .replace(/\s+/g, ' ').trim();
      }
      function wordSet(s) {
        return new Set(s.split(' ').filter(function(w){ return w.length > 1; }));
      }
      function wordScore(a, b) {
        var wa = wordSet(a), wb = wordSet(b);
        if (wa.size === 0 || wb.size === 0) return 0;
        var overlap = 0;
        wa.forEach(function(w){ if(wb.has(w)) overlap++; });
        // Score based on smaller set — if all words from the shorter name match, it's a good match
        var minSize = Math.min(wa.size, wb.size);
        return minSize > 0 ? overlap / minSize : 0;
      }

      var contactNorm = normName(acctName);
      var bestScore = 0, bestKey = null;
      Object.keys(accountData).forEach(function(k) {
        var score = wordScore(contactNorm, normName(k));
        if (score > bestScore) { bestScore = score; bestKey = k; }
      });
      // Require: (50%+ overlap of smaller name AND 2+ matching words) OR (100% of smaller name)
      var contactWords = wordSet(contactNorm);
      var bestWords = bestKey ? wordSet(normName(bestKey)) : new Set();
      var overlapCount = 0;
      contactWords.forEach(function(w){ if(bestWords.has(w)) overlapCount++; });
      if (bestKey && ((bestScore >= 0.5 && overlapCount >= 2) || bestScore >= 1.0)) {
        account = accountData[bestKey];
        matchMethod = 'fuzzy-' + Math.round(bestScore*100) + '%';
      }
    }

    // 6. Phone number match as last resort
    if (!account && phone) {
      var normPhone = phone.replace(/\D/g, '').slice(-10); // last 10 digits
      if (normPhone.length >= 10) {
        var phoneKey = Object.keys(accountData).find(function(k) {
          var ap = (accountData[k].phone || '').replace(/\D/g, '').slice(-10);
          return ap.length >= 10 && ap === normPhone;
        });
        if (phoneKey) {
          account = accountData[phoneKey];
          matchMethod = 'phone';
        }
      }
    }
    ...
    if (account) {
      ...
      unmatched++;
      unmatchedNames.add(acctName || mailingStreet || fullName);
    }
  });
  ...
  if (unmatchedNames.size > 0 && unmatchedNames.size <= 30) {
    console.log('[Sheets Loader] Unmatched contacts:', Array.from(unmatchedNames));
  }
}
...
function parseOrders(rows) {
  ...
  rows.filter(r => r['Order #'] || r['Client']).forEach((r, i) => {
    ...
    // Update stage/dates — always take the most recent/authoritative value
    // Cultivera may update status on individual line items, so we want the latest
    const o = orderMap[orderNum];
    if (stage) o.s = stage;  // Always update stage — last line item wins (they should all match)
    if (r['Client']) o.a = r['Client'];
    if (r['Assigned To']) o.dm = r['Assigned To'];
    if (normDate(r['Submitted Date'])) o.sd = normDate(r['Submitted Date']);
    if (normDate(r['Estimated delivery date'] || r['Transfer Date'])) o.d = normDate(r['Estimated delivery date'] || r['Transfer Date']);
    if (normDate(r['Estimated delivery date'])) o.dd = normDate(r['Estimated delivery date']);
    if (normDate(r['Manifested Date'])) o.pd = normDate(r['Manifested Date']);
    if (r['License #']) o.license = r['License #'];
    if (r['Discount']) o.discount = r['Discount'];
    if (r['Submitted By']) o.submittedBy = r['Submitted By'];
    if (r['Manifest Reference #']) o.manifestRef = r['Manifest Reference #'];
    if (r['Invoiced']) o.invoiced = r['Invoiced'];
    if (r['Invoiced By']) o.invoicedBy = r['Invoiced By'];
    if (r['Release Transaction #']) o.releaseRef = r['Release Transaction #'];
  ...
  opps.forEach(o => {
    // Recalculate revenue from actual line items (ensures accuracy if items were added/removed)
    o.r = Math.round(o.lineItems.reduce(function(s, li) { return s + (li.amount || 0); }, 0) * 100) / 100;
...
// Col layout from the Product Allocation sheet (verified via API):
//  0: Week of       1: Room    2: Totes   3: Plant Count   4: Total Days
//  5: Strain        6: Flower Delivery    7: Joint Delivery
//  8: Per Tote      9: Actual Per Tote
// 10: Proj. 3.5g   11: Actual 3.5g (col L)   12: Target 7g
// 13: Target 28g   14: Wholesale Tier 1   15: Proj. Micros 14g
// 16: Actual Micros (col Q)   17: Wholesale Micros   18: Projected Trim
// 19: % Trim   20: 1g joints   21: 2pk   22: 10pk
// 23: Infused checkbox   24: Batch Count   25: Total Proj 1g   26: Total Proj 0.5g
// 27: Fresh Frozen checkbox (= Live Resin)   28: Cured checkbox
// ─────────────────────────────────────────────
...
function parseInventory(rawRows) {
  ...
  const results = dataRows
    ...
    .map(r => {
      ...
      // Col mapping (from actual Product Allocation sheet — verified via API):
      //  0: Week of date     1: Room       2: Totes      3: Plant Count  4: Total Days
      //  5: Strain           6: Flower Del 7: Joint Del
      //  8: Per Tote         9: Actual Per Tote
      // 10: Proj. 3.5g     11: Actual 3.5g (col L)
      // 12: Target 7g      13: Target 28g    14: Wholesale Tier 1 Flower
      // 15: Proj. Micros   16: Actual Micros 14g (col Q)   17: Wholesale Micros
      // 18: Projected Trim 19: % Trim
      // 20: 1g joints      21: 2pk joints    22: 10pk joints
      // 23: Infused checkbox (TRUE/FALSE) — infused prerolls available
      // 24: Batch Count    25: Total Proj 1g   26: Total Proj 0.5g
      // 27: Fresh Frozen checkbox (TRUE/FALSE) — going to live resin
      // 28: Cured checkbox

      var proj35   = Math.round(fnum(r[10]));  // Projected 3.5g
      var actual35 = Math.round(fnum(r[11]));  // Actual 3.5g (col L) — 0 if not yet packaged
      var projMicro  = Math.round(fnum(r[15])); // Projected micros
      var actualMicro = Math.round(fnum(r[16])); // Actual micros (col Q) — 0 if not yet packaged

      // Use actuals when available, fall back to projected
      var best35 = actual35 > 0 ? actual35 : proj35;
      var bestMicro = actualMicro > 0 ? actualMicro : projMicro;

      var proj1g  = Math.round(fnum(r[20]));   // 1g prerolls
      var proj2pk = Math.round(fnum(r[21]));   // 2pk joints
      var proj10pk = Math.round(fnum(r[22]));  // 10pk joints
      var infused = String(r[23] || '').toUpperCase() === 'TRUE';       // Infused prerolls
      var isFreshFrozen = String(r[27] || '').toUpperCase() === 'TRUE'; // Fresh Frozen = Live Resin
      ...
      return {
        ...
        fd:      normDate(r[6]),
        jd:      normDate(r[7]),
        p35:     best35,          // Use actual if available, else projected
        p35proj: proj35,          // Always keep projected for reference
        p35actual: actual35,      // Actual packaged count
        pm:      bestMicro,       // Use actual if available, else projected
        pmProj:  projMicro,       // Always keep projected for reference
        pmActual: actualMicro,    // Actual packaged count
        j1:      proj1g,          // 1g prerolls
        j2:      proj2pk,         // 2pk joints
        j10:     proj10pk,        // 10pk joints
        inf:     isFreshFrozen ? 1 : 0,  // Fresh Frozen = going to live resin
        infused: infused ? 1 : 0,        // Infused prerolls available
        perTote:      fnum(r[8]),
        actualPerTote: fnum(r[9]),
        target7g:     num(r[12]),
        target28g:    num(r[13]),
        wholesale:    num(r[14]),
        wholesaleMicro: num(r[17]),
        trim:         fnum(r[18]),
        freshFrozen:  isFreshFrozen ? 1 : 0,
        qa:           0,
        cured:        String(r[28] || '').toUpperCase() === 'TRUE' ? 1 : 0,
      };
    });
...
function parseCurrentInventory(rows) {
  return rows
    ...
    .map(r => {
      ...
      var subProdLine = r['Sub-Product-Line'] || '';
      var category = r['Category'] || '';
      var prodLine = r['Product-Line'] || '';

      // Extract strain from product name: "2pk - 01.0 Gram - Preroll - Blue Dream" → "Blue Dream"
      // or "14.0 Gram - Micro Buds - Grape Cake" → "Grape Cake"
      var parts = product.split(' - ');
      var strain = parts.length >= 4 ? parts.slice(3).join(' - ').trim()
                 : parts.length >= 3 ? parts.slice(2).join(' - ').trim()
                 : parts.length >= 2 ? parts[parts.length - 1].trim()
                 : product;

      // Determine product type from product name and category
      var productType = 'Other';
      if (/^2pk\b/i.test(product)) productType = '2pk Joints';
      else if (/^10pk\b/i.test(product)) productType = '10pk Joints';
      else if (/hash.*preroll|hash.*joint/i.test(product)) productType = 'Hash Joints';
      else if (/preroll|01\.0 gram.*preroll|1\.0 gram.*preroll/i.test(product)) productType = '1g Joints';
      else if (/micro|14\.0 gram/i.test(product)) productType = 'Micro 14g';
      else if (/flower|3\.5/i.test(product) || /flower/i.test(category)) productType = 'Flower 3.5g';
      else if (/vape|cart/i.test(product) || /vape/i.test(category)) productType = 'Vape';
      else if (/rosin|hash|concentrate/i.test(product) || /rosin|hash|concentrate/i.test(category)) productType = 'Concentrate';

      return {
        barcode:        r['Barcode'],
        product:        product,
        productLine:    prodLine,
        subProductLine: subProdLine,
        category:       category,
...
function enrichAccountsAndBuildKPIs(accountData, opps) {
  ...
  opps.forEach(o => {
    ...
    // If still no match, try fuzzy name matching against existing accounts
    if (!accountData[name] && name) {
      function normAcctName(n) {
        return n.toLowerCase()
          .replace(/[()[\]{}\-_.,'"]/g, ' ')
          .replace(/\b(cannabis|marijuana|recreational|dispensary|llc|inc|co|corp|the|of|and|&|club|company|store|shop|house|weed|pot|market)\b/gi, ' ')
          .replace(/[^a-z0-9\s]/g, ' ')
          .replace(/\s+/g, ' ').trim();
      }
      var normOrder = normAcctName(name);
      var orderWords = new Set(normOrder.split(' ').filter(function(w){ return w.length > 1; }));
      if (orderWords.size >= 1) {
        var bestScore = 0, bestMatch = null;
        Object.keys(accountData).forEach(function(k) {
          var normK = normAcctName(k);
          var kWords = new Set(normK.split(' ').filter(function(w){ return w.length > 1; }));
          if (kWords.size === 0) return;
          var overlap = 0;
          orderWords.forEach(function(w){ if(kWords.has(w)) overlap++; });
          var score = overlap / Math.min(orderWords.size, kWords.size);
          if (score > bestScore && overlap >= 2) { bestScore = score; bestMatch = k; }
        });
    ...
    // If still no match, create a stub account — but ONLY for orders from 2023+
    if (!accountData[name] && name) {
      // Check if this order has a close date from 2023 or later
      var stubOrderDate = o.d || o.sd || '';
      var orderYear = stubOrderDate ? parseInt(stubOrderDate.slice(0, 4)) : 0;
      if (orderYear < 2023) return; // skip old orders, don't create stubs
    ...
    var oDate = o.sd || o.d || '';
    if (oDate && (!acct.stats.lastOrder || oDate > acct.stats.lastOrder)) {
      acct.stats.lastOrder = oDate;
    }
...
function injectIntoApp(accountData, kpis, opps, inventory, currentInv, pipeline, sheetTasks) {
  ...
  if (sheetTasks && sheetTasks.length && typeof window.tasks !== 'undefined') {
    ...
    var doneMap = {};
    try { doneMap = JSON.parse(localStorage.getItem('rb_tasks_done')||'{}'); } catch(e) {}
    var doneApplied = 0;
    ...
    if (doneApplied) console.log('[Sheets Loader] Applied localStorage done state to', doneApplied, 'tasks');
  }
  ...
  try { if (typeof checkSyncStatus === 'function')       checkSyncStatus(); }       catch(e) {}
  ...
  try {
    ...
    setTimeout(async function() {
      try {
        ...
        const sheetTasks = parseTasks(taskRows);
