...
function parseInventory(rawRows) {
  ...
  const results = dataRows
    ...
    .map(r => {
      ...
      var projJoints = Math.round(fnum(r[19]));
      var infAvail = String(r[28] || '').toUpperCase() === 'TRUE';
      ...
      return {
        ...
        // Joints: col 19 is the total projected joints — not broken out by pack
        // We'll put the total in j1 for now (the CRM sums j1+j2+j10 for display)
        j1:      projJoints,
        j2:      0,
        j10:     0,
        inf:     infAvail ? 1 : 0,  // boolean flag — 1 if infusions available
        perTote:      num(r[3]),
...
function enrichAccountsAndBuildKPIs(accountData, opps) {
  ...
  Object.keys(accountData).forEach(name => {
    ...
    // Check if they have any order in the past 3 years
    const lastOrder = acct.stats.lastOrder || '';
    if (lastOrder >= cutoff3yr) return; // Has recent activity — keep

    // No orders in 3 years and not a real account — remove
...
function injectIntoApp(accountData, kpis, opps, inventory, currentInv, pipeline, sheetTasks) {
  ...
  window.ACCOUNT_DATA  = accountData;
  window.ACCT_KPIS     = kpis;
  window.OPPS_DATA     = opps;
  window.INV_DATA      = inventory;
  window.CURRENT_INV_DATA = currentInv;
  window.PIPELINE_DATA = pipeline;
  ...
  if (sheetTasks && sheetTasks.length && typeof window.tasks !== 'undefined') {
    ...
    sheetTasks.forEach(t => {
      ...
      if (!existing.has(key)) {
        // Apply saved done state
        var doneKey = key.toLowerCase();
        if (doneMap[doneKey]) t.done = true;
        window.tasks.push(t);
        existing.add(key);
        added++;
      }
