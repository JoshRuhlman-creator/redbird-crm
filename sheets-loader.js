    ...
    if (!account && acctName) {
      ...
      function normName(n) {
        return n.toLowerCase()
          ...
          .replace(/\b(cannabis|marijuana|recreational|dispensary|llc|inc|co|the|of|and|&)\b/gi, ' ')
      ...
      function wordScore(a, b) {
        // How many words from a appear in b, and vice versa
        var wa = wordSet(a), wb = wordSet(b);
        var overlap = 0;
        wa.forEach(function(w){ if(wb.has(w)) overlap++; });
        var total = Math.max(wa.size, wb.size);
      ...
      // Require at least 60% word overlap to match
