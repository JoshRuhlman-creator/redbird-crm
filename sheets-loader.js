...
function parseAccounts(rows) {
  ...
  // ── HELPER: strip "[LICENSE]" suffix from Cultivera-style names ──
  // Cultivera sometimes exports Client as "The Kushery [430774]" with the
  // license number appended in brackets. The License # column already carries
  // that value separately, so the bracketed suffix is redundant — and worse,
  // it splits one retailer into two accounts ("The Kushery" vs "The Kushery
  // [430774]"). Strip it so all orders for that retailer collapse onto a
  // single canonical account name.
  function _stripLicenseSuffix(s) {
    return String(s || '').replace(/\s*\[\d{5,}\]\s*$/, '').trim();
  }

  const tradeNameTotal = {};
  rows.filter(r => r['Trade Name']).forEach(r => {
    const tn = _stripLicenseSuffix(r['Trade Name']);
    if (tn) tradeNameTotal[tn] = (tradeNameTotal[tn] || 0) + 1;
  });

  rows.filter(r => r['Trade Name']).forEach(r => {
    const tradeName = _stripLicenseSuffix(r['Trade Name']);
    if (!tradeName) return;
...
function parseOrders(rows) {
  ...
  rows.filter(r => r['Order #'] || r['Client']).forEach((r, i) => {
    ...
    // Strip "[LICENSE]" suffix Cultivera appends to some Client names — keeps
    // "The Kushery" and "The Kushery [430774]" from becoming two accounts.
    // The License # column already carries the value separately.
    function stripBracketedLicense(s) {
      return String(s || '').replace(/\s*\[\d{5,}\]\s*$/, '').trim();
    }
    const clientName = stripBracketedLicense(r['Client']);
    ...
    if (!o.a && r['Client']) o.a = stripBracketedLicense(r['Client']);
...
function enrichAccountsAndBuildKPIs(accountData, opps) {
  ...
  const ACCOUNT_ALIASES = {
    ...
    'StonR': 'Stonr',
    'mary jane': 'Mary Jane',
    // Smoking Crow → Organic Liberty LLC. The shop was relicensed/renamed
    // mid-history: 145 older orders (Cultivera Client "Smoking Crow", blank
    // license) belong on the same account as the current orders
    // (Client "ORGANIC LIBERTY LLC (SMOKING CROW)", license 437560).
    // Confirmed same shop by the user.
    'Smoking Crow': 'ORGANIC LIBERTY LLC (SMOKING CROW)',
  };
  ...
  opps.forEach(o => {
    ...
    // "Carrying" a family means the account has PAID for that family — i.e.
    // placed a non-sample order line in it. A family the account has only ever
    // been SAMPLED (never bought) must NOT count as carried, otherwise the
    // "sampled X" and "missing X" filters become mutually exclusive: sampling
    // a vape would wrongly mark the account as carrying vapes.
    // So we scan line items directly and skip sample lines + zero-value lines.
    if (o.r > 0 && o.lineItems) {
      o.lineItems.forEach(li => {
        if (li.sample) return;                       // sample line — not "carried"
        if (!(li.amount > 0)) return;                // zero-value line — not a real purchase
        const fam = li.family;
    ...
    // Revenue KPIs — count every FULFILLED order, not just "Closed Won".
    // An order's lifecycle is Preorder → Submitted → Purchase Order →
    // Sublotted → Delivered → Closed Won → Invoiced. "Invoiced" is the FINAL
    // stage (delivered AND billed) — the most-complete revenue there is.
    // Counting only "Closed Won" silently dropped revenue for every account
    // whose orders had progressed to Invoiced/Delivered/Sublotted/Manifested.
    // This set matches the FULFILLED stages used throughout crm.html.
    if (/closed.won|invoiced|delivered|sublotted|manifested/i.test(o.s)) {
  ...
  // ── Rename license-routed accounts to their Cultivera Client name ──
  // Two cases this handles:
  //   1. Multi-location stores keyed as "Trade Name [license]" by parseAccounts
  //      → renamed to their Cultivera Client string.
  //   2. Single-location accounts whose CRM Accounts Trade Name differs from
  //      the Cultivera Client name (e.g. CRM has "Sativa Sisters" but Cultivera
  //      reports "Sativa Sisters Spokane"; or "Sativa Sisters Ii" vs
  //      "Sativa Sisters Clarkston"). When a license's orders overwhelmingly
  //      carry one Client name that differs from the current account key, the
  //      account is renamed to match Cultivera.
  //
  // Runs in BOTH Phase 1 and Phase 2 — idempotent (once renamed, the new key
  // already equals the Client name so the pass no-ops).
  (function renameLicenseAccounts() {
    ...
    var totalByKey = {};        // acctKey → total orders counted
    ...
    Object.keys(accountData).forEach(function(oldKey) {
      var isMultiLoc = /\s\[.+\]$/.test(oldKey);  // "Trade Name [license]" form
      var newKey = '';
      var counts = clientNamesByKey[oldKey];

      if (counts && Object.keys(counts).length) {
        // Most common Cultivera Client name + how dominant it is
        var sorted = Object.keys(counts).sort(function(a,b){ return counts[b]-counts[a]; });
        var topName = sorted[0];
        var topShare = counts[topName] / (totalByKey[oldKey] || 1);

        if (isMultiLoc) {
          // Multi-location key — always rename to the dominant Client name.
          newKey = topName;
        } else {
          // Single-location account. Only rename if the Cultivera Client name
          // DIFFERS from the current key AND is strongly dominant (≥80% of
          // orders carry it). The 80% gate prevents renaming on a stray
          // mis-tagged order. This is what fixes "Sativa Sisters" →
          // "Sativa Sisters Spokane".
          if (topName !== oldKey && topShare >= 0.8) {
            newKey = topName;
          }
        }
      }

      // Fallback for multi-location keys with no _rawClient orders (e.g. a
      // store with only historical opps): build "Trade Name (city)".
      if (!newKey && isMultiLoc) {
  ...
  // ── Display-only title-case normalization ──
  // Cultivera Client names are stored inconsistently — some all-caps
  // ("HAVE A HEART Skyway"), some mixed ("Cannazone Burlington"), some title
  // ("Western Bud Bellingham"). After the rename pass adopts these names as
  // account keys, the account list becomes hard to scan: same brand fragments
  // across A–Z sort (caps sort before lowercase), and look-alike accounts are
  // easy to miss. This pass title-cases keys that are entirely or mostly caps,
  // preserving small acronyms and intentional casing.
  //
  // Examples:
  //   "HAVE A HEART Skyway" → "Have A Heart Skyway"
  //   "CANNAZONE"           → "Cannazone"
  //   "SHAWN KEMP'S CANNABIS BELLTOWN" → "Shawn Kemp's Cannabis Belltown"
  //
  // Skipped:
  //   "Uncle Ike's"  (already mixed — no change)
  //   "MJ's Place"   (acronym MJ preserved by the all-caps-word rule below)
  //   anything ≤3 chars (could be a real acronym/initialism — leave alone)
  (function normalizeAccountCasing() {
    // Tokens we want to preserve as ALL CAPS (or specific casing) — common
    // legal/biz acronyms that look wrong title-cased. Anything not in here
    // gets straight title-case: first letter cap, rest lower.
    var ACRONYMS = {
      'llc':'LLC', 'inc':'Inc', 'co':'Co', 'corp':'Corp',
      'cc':'CC', 'pllc':'PLLC', 'lp':'LP',
      'ii':'II', 'iii':'III', 'iv':'IV',
      'nw':'NW', 'ne':'NE', 'sw':'SW', 'se':'SE',
      'usa':'USA', 'us':'US',
      'mlk':'MLK',  // street name abbreviation
    };
    function toTitleCase(s) {
      return String(s).split(/(\s+|-|\(|\)|\/)/).map(function(token) {
        if (!/[a-z]/i.test(token)) return token;
        var lc = token.toLowerCase();
        // Known acronym? use the canonical form
        if (Object.prototype.hasOwnProperty.call(ACRONYMS, lc)) return ACRONYMS[lc];
        // Standard title-case: first char upper, rest lower; preserve "'s"
        return lc.charAt(0).toUpperCase() + lc.slice(1);
      }).join('');
    }
    function shouldNormalize(name) {
      // >60% of the alphabetic chars are uppercase = "shouty" name
      var letters = name.replace(/[^a-z]/gi, '');
      if (letters.length < 4) return false;
      var upperCount = name.replace(/[^A-Z]/g, '').length;
      return (upperCount / letters.length) > 0.6;
    }

    Object.keys(accountData).forEach(function(oldKey) {
      if (!shouldNormalize(oldKey)) return;
      var newKey = toTitleCase(oldKey);
      if (newKey === oldKey || accountData[newKey]) return;
      accountData[newKey] = accountData[oldKey];
      delete accountData[oldKey];
      if (kpis[oldKey]) { kpis[newKey] = kpis[oldKey]; delete kpis[oldKey]; }
      var lic = (accountData[newKey] && accountData[newKey].license) || '';
      if (lic && window.LICENSE_TO_ACCT) {
        window.LICENSE_TO_ACCT[String(lic).trim()] = newKey;
      }
      opps.forEach(function(o) { if (o.a === oldKey) o.a = newKey; });
    });
  })();
...
function buildPipelineData(opps) {
  ...
  opps.forEach(o => {
    ...
    // Summary stats — count all FULFILLED stages as realized revenue
    // (matches the KPI calc and crm.html's FULFILLED set). "Invoiced" is the
    // final delivered+billed stage and must be included.
    if (/closed.won|invoiced|delivered|sublotted|manifested/i.test(o.s)) {
    ...
    if (/purchase.order|submitted|preorder/i.test(o.s)) {
...
function mergeAccountsByAddress(accountData, kpis, opps) {
  function normAddrKey(a) {
    var rawStreet = String(a.address || '').trim();
    var street = rawStreet.toLowerCase()
      .replace(/[.,]/g, '')
    ...
    // STRICT validity gate — address-merge has caused false merges of
    // unrelated businesses when address data is blank/garbled (e.g. the City
    // column holding a street address). Only treat an address as mergeable if:
    //   1. The street starts with a house/building number, then has letters
    //      (a real "123 Main St" shape — not a bare city name or blank)
    //   2. A 5-digit zip is present (second independent signal)
    //   3. City is non-empty
    // If any fail, return null → this account simply won't address-merge.
    // License-based merging (separate, exact) still covers the safe cases.
    if (!street || !city) return null;
    if (!/^\d+\s+\S/.test(street)) return null;       // must start "<number> <something>"
    if (!/^\d{5}$/.test(zip)) return null;            // must have a real 5-digit zip
    if (street.length < 6) return null;               // reject absurdly short
  ...
  // Loose name similarity — used ONLY to gate address merges. Two accounts at
  // the same address with totally unrelated names (e.g. "Cannazone" vs
  // "Evolve Washington") are NOT merged: same address can mean a sublease, a
  // relocated tenant, or simply bad address data. Returns true if the names
  // share enough of a token to plausibly be the same business.
  function namesPlausiblySame(a, b) {
    function toks(s) {
      return String(s).toLowerCase()
        .replace(/\b(llc|inc|co|company|cannabis|marijuana|the|of|and)\b/g, '')
        .replace(/[^a-z0-9 ]/g, '')
        .split(/\s+/).filter(function(t){ return t.length >= 3; });
    }
    var ta = toks(a), tb = toks(b);
    if (!ta.length || !tb.length) return false;
    // Shared significant token, OR one name is a prefix of the other
    var setB = {};
    tb.forEach(function(t){ setB[t] = 1; });
    for (var i = 0; i < ta.length; i++) {
      if (setB[ta[i]]) return true;
    }
    var na = ta.join(''), nb = tb.join('');
    return na.indexOf(nb) === 0 || nb.indexOf(na) === 0;
  }

  function addMergeGroup(names, requireNameMatch) {
    ...
    if (requireNameMatch) {
      // Address-based group: only keep losers whose name plausibly matches the
      // winner. A mismatched name at the same address is left as its own
      // account rather than wrongly absorbed.
      losers = losers.filter(function(l) {
        var ok = namesPlausiblySame(winner, l);
        if (!ok) {
          console.warn('[Sheets Loader] Skipped address-merge of "' + l + '" into "' + winner + '" — names too different');
        }
        return ok;
      });
    ...
    [winner].concat(losers).forEach(function(n) { claimed[n] = true; });
    merges.push({ winner: winner, losers: losers });
  }

  // License groups first — highest confidence, NO name check (license is
  // authoritative: identical WSLCB license = same store, period).
  Object.keys(byLicense).forEach(function(lic) {
    if (byLicense[lic].length >= 2) addMergeGroup(byLicense[lic], false);
  });
  // Then address groups — these REQUIRE a plausible name match to merge.
  Object.keys(byAddr).forEach(function(key) {
    if (byAddr[key].length >= 2) addMergeGroup(byAddr[key], true);
  });
  ...
  // ── Diagnostic: likely-duplicate pairs the merger couldn't auto-resolve ──
  // These are accounts whose names overlap heavily but DIDN'T merge because
  // they have different (or blank) licenses and different addresses — the
  // Smoking Crow / Organic Liberty LLC pattern. The merger correctly didn't
  // touch them, but you might want to review and add aliases to ACCOUNT_ALIASES.
  // Stored on window so you can re-print with: console.table(window.LIKELY_DUPES)
  try {
    function nameTokens(s) {
      return String(s||'').toLowerCase()
        .replace(/\b(llc|inc|co|company|cannabis|marijuana|the|of|and)\b/g, '')
        .replace(/[^a-z0-9 ]/g, '')
        .split(/\s+/).filter(function(t){ return t.length >= 4; });
    }
    var allNames = Object.keys(accountData);
    var pairs = [];
    var seen = {};
    allNames.forEach(function(a) {
      var ta = nameTokens(a);
      if (!ta.length) return;
      allNames.forEach(function(b) {
        if (a === b) return;
        var key = a < b ? a + '|' + b : b + '|' + a;
        if (seen[key]) return;
        seen[key] = true;
        var tb = nameTokens(b);
        if (!tb.length) return;
        // Share at least 2 significant tokens, OR a single rare-ish 5+ char token
        var shared = ta.filter(function(t){ return tb.indexOf(t) >= 0; });
        var rareShared = shared.filter(function(t){ return t.length >= 5; });
        if (shared.length < 2 && rareShared.length < 1) return;
        // Skip if the merger already collapsed them (one no longer exists)
        if (!accountData[a] || !accountData[b]) return;
        // Skip if they DID share a license (the merger would have handled it)
        var la = String((accountData[a]||{}).license||'').trim();
        var lb = String((accountData[b]||{}).license||'').trim();
        if (la && lb && la === lb) return;
        var ordersA = (kpis[a] && kpis[a].oppCount) || 0;
        var ordersB = (kpis[b] && kpis[b].oppCount) || 0;
        pairs.push({
          accountA: a, ordersA: ordersA, licenseA: la || '(blank)',
          accountB: b, ordersB: ordersB, licenseB: lb || '(blank)',
          sharedTokens: shared.join(', '),
        });
      });
    });
    window.LIKELY_DUPES = pairs;
    if (pairs.length) {
      console.log('[Sheets Loader] ' + pairs.length + ' likely-duplicate pairs flagged for review. Inspect: console.table(window.LIKELY_DUPES)');
    }
  } catch(e) {
    console.warn('[Sheets Loader] Duplicate diagnostic failed:', e);
  }
  ...
  try {
    ...
    setTimeout(async function() {
      try {
        ...
        // ── Phase 3: Build invoice data for Reports ──
        // The old "invoicedetails" sheet tab is fed by a separate auto-refresh
        // job that stopped updating after March 2026 — so the Reports tab was
        // stuck in March. Instead we now DERIVE invoice data directly from the
        // already-parsed, fully-merged OPPS_DATA (live + historical orders).
        // This is always current because it's the same data the rest of the
        // CRM runs on: license-routed to clean account names, family-tagged,
        // and using the FULFILLED-stage definition.
        //
        // Each fulfilled order's line items become invoice rows. We exclude
        // sample lines (they aren't revenue) and zero-value lines.
        setTimeout(function() {
          try {
            showBanner('⟳ Building invoice data for reports…', '#7c3aed');
            var src = (typeof OPPS_DATA !== 'undefined' && OPPS_DATA) ? OPPS_DATA
                      : (window.OPPS_DATA || []);
            var invData = [];
            // Stage inclusion rules (per Josh, May 2026):
            //   • FULFILLED_HARD — always count: order has shipped/billed/delivered.
            //   • PIPELINE_WITH_DATE — count only when estimated delivery date is
            //     populated (column L → o.dd), so the order has a committed month.
            //   • Everything else (Preorder, Purchase Order, open, Pending, etc.)
            //     is excluded — pre-commitment or noise from the historical
            //     SF opps merge ("open" is not a Cultivera status).
            var FULFILLED_HARD     = /closed.won|invoiced|delivered|manifested/i;
            var PIPELINE_WITH_DATE = /sublotted|submitted/i;
            src.forEach(function(o) {
              if (!o) return;
              var stage     = o.s || '';
              var hasDelDate = !!(o.dd && String(o.dd).length >= 7);
              var counts = FULFILLED_HARD.test(stage)
                        || (PIPELINE_WITH_DATE.test(stage) && hasDelDate);
              if (!counts) return;
              // Date the invoice by delivery date, then close, then submitted.
              var dateStr = o.dd || o.d || o.sd || '';
              if (!dateStr) return;
              var dt = new Date(dateStr);
              if (isNaN(dt)) return;
              // Close Month as "M/1/YYYY" to match what renderReports expects
              var month = (dt.getMonth() + 1) + '/1/' + dt.getFullYear();
              var items = o.lineItems || [];
              if (!items.length) {
                // Order with no line-item detail — emit a single summary row
                invData.push({
                  account: o.a || '', opp: o.n || '',
                  amount: o.r || 0, price: o.r || 0, qty: 0,
                  family: o.family || 'Other', month: month,
                });
                return;
              }
              items.forEach(function(li) {
                if (li.sample) return;                 // samples aren't revenue
                var amt = li.amount || 0;
                if (!(amt > 0)) return;                // skip zero-value lines
                invData.push({
                  account: o.a || '',
                  opp:     o.n || '',
                  amount:  amt,
                  price:   amt,
                  qty:     li.units || 0,
                  family:  li.family || 'Other',
                  month:   month,
                });
              });
            });
            window.INVOICE_DATA = invData;
            console.log('[Sheets Loader] Phase 3 — Invoice data built from OPPS_DATA:', invData.length, 'rows from', src.length, 'orders');
            ...
            console.warn('[Sheets Loader] Invoice data build error (non-fatal):', err);
