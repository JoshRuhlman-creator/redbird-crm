...
//
// New CRM Accounts schema (post-rebuild):
//   Trade Name | Mailing Street | Mailing City | Mailing State | Mailing Zip
//   | Source | First Order | Last Order | Order Count | Notes | Manual Edit
// ─────────────────────────────────────────────
...
function parseAccounts(rows) {
  ...
  rows.filter(r => r['Trade Name']).forEach(r => {
    ...
    // No more license-based disambiguation — CRM Accounts is already deduped
    // by lowercase trade name during the rebuild.
    accountData[tradeName] = {
      // Address fields
      address:     r['Mailing Street'] || '',
      city:        r['Mailing City']   || '',
      state:       r['Mailing State']  || '',
      zip:         r['Mailing Zip']    || '',
      ...
      lastOrderSf: r['Last Order']    || '',  // Salesforce-derived; OrdersLog will override below
      orderCount:  parseInt(r['Order Count']) || 0,
      notes:       r['Notes']         || '',
...
function enrichAccountsAndBuildKPIs(accountData, opps) {
  ...
  opps.forEach(o => {
    ...
    if (!accountData[name] && name && acctNamesLower[name.toLowerCase()]) {
  ...
  // ── Cleanup: Remove stub accounts (created by parseOrders fallback) with no orders in 3 years ──
  ...
  if (removed > 0) {
    console.log(`[Sheets Loader] Cleaned up ${removed} stale stub accounts (no orders in 3 years, not in CRM Accounts)`);
