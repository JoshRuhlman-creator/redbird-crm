...
// ─────────────────────────────────────────────
// RETAILER SALES (Top Shelf Data — WA retail context)
// Builds window.RETAIL_SALES keyed by license #.
// Stores per-year monthly-average retail sales (avg2024, avg2025, avg2026).
// Fire-and-forget: if the tab doesn't exist yet, the drill panel just shows '—'.
// ─────────────────────────────────────────────
function loadRetailerSales() {
  fetchTab(SHEETS_CONFIG.tabs.retailerSales).then(function(rows) {
    var lookup = {};
    ...
    rows.forEach(function(r) {
      ...
      lookup[lic] = {
        ...
        county:  r['County'] || '',
        address: r['Address'] || '',
        avg2024: parseMoney(r['AvgMonthly 2024']),
        avg2025: parseMoney(r['AvgMonthly 2025']),
        avg2026: parseMoney(r['AvgMonthly 2026'])
      };
    });
    window.RETAIL_SALES = lookup;
    console.log('[Sheets Loader] RetailerSales loaded:', Object.keys(lookup).length, 'retailers');
  }).catch(function(err) {
    console.warn('[Sheets Loader] RetailerSales tab not loaded (non-fatal):', err.message);
    window.RETAIL_SALES = window.RETAIL_SALES || {};
  });
}
...
const CURRENT_INV_ROOMS = ['9. Packaged Inventory', '8. Label Ready'];
...
function parseCurrentInventory(rows) {
  ...
  return rows
    // Sellable rooms: finished goods + product waiting on labels.
    // Label Ready rows are flagged (labelReady) so the CRM can tag them.
    // Bulk lots are never sellable current inventory, whatever room they sit in.
    .filter(r => {
      if (!(r['Product'] || r['Barcode'])) return false;
      const room = (r['Room'] || '').toString().trim();
      if (CURRENT_INV_ROOMS.indexOf(room) === -1) return false;
      if (/^bulk\b/i.test((r['Product'] || '').toString().trim())) return false;
      return numCell(r['Units For Sale']) > 0;
    })
    .map(r => ({
      barcode:        r['Barcode'],
      product:        r['Product'],
      ...
      labelReady:     (r['Room'] || '').toString().trim() === '8. Label Ready',
    }));
}
...
async function loadSheetsData() {
  ...
  // Fire-and-forget retailer sales load — populates window.RETAIL_SALES.
  // Used by the Revenue Map drill-down to show each store's WA retail sales.
  loadRetailerSales();
        ...
        setTimeout(function() {
          try {
            ...
            var src = (typeof OPPS_DATA !== 'undefined' && OPPS_DATA) ? OPPS_DATA
                      : (window.OPPS_DATA || []);
            var invData = [];
            var FULFILLED = /closed.won|invoiced|delivered|sublotted|manifested/i;
            src.forEach(function(o) {
              if (!o || !FULFILLED.test(o.s || '')) return;
